...
def configure(keymap):
    # Emacs キーバインド適用外のアプリケーションを指定
    notKeyBindList = ["mintty.exe"]

    # 親指 Ctrl 適用のアプリケーションを指定
    thumCtrlList = ["mintty.exe"]



    # Emacs キーバインドを適用するキーマップを定義
    def IsKeyBind(window):
        if window.getProcessName() in notKeyBindList:
            return False
        else:
            return True
    keymapGlobal = keymap.defineWindowKeymap(check_func=IsKeyBind)

    # 親指 Ctrl を適用するキーマップを定義
    def IsThumbCtrl(window):
        if window.getProcessName() in thumCtrlList:
            return True
        else:
            return False
    keymapTerminal = keymap.defineWindowKeymap(check_func=IsThumbCtrl)



    # 独自モディフィアキーを定義
    keymap.defineModifier(29, "U0")
    ...
    keymapGlobal["O-(29)"] = ImeOff
    keymapGlobal["O-(28)"] = ImeOn
    keymapTerminal["O-(29)"] = ImeOff
    keymapTerminal["O-(28)"] = ImeOn



    # 親指 Ctrl のキーマップ
    terminalKeyList = ("("+str(code)+")" for code in range(256))
    for key in terminalKeyList:
        keymapTerminal["U0-" + key] = "C-" + key
        keymapTerminal["U1-" + key] = "A-" + key
        keymapTerminal["U0-S-" + key] = "C-S-" + key
        keymapTerminal["U1-S-" + key] = "A-S-" + key
        keymapTerminal["U0-U1-" + key] = "C-A-" + key
        keymapTerminal["U0-U1-S-" + key] = "C-A-S-" + key



    # 範囲選択
    keymapGlobal.markFlag = False
    def ToggleMark():
        keymapGlobal.markFlag = 1 - keymapGlobal.markFlag
    def MarkOff():
        keymapGlobal.markFlag = 0
    def MarkOn():
        keymapGlobal.markFlag = 1
    keymapGlobal["U0-Space"] = ToggleMark


    # キャンセル
    def Cancel():
        MarkOff()
    keymapGlobal["U0-G"] = Cancel


    # 移動系のキーマップ
    def Move(key):
        if keymapGlobal.markFlag == True:
            keymap.command_InputKey("S-" + key)()
        else:
            keymap.command_InputKey(key)()

    def Right():
        Move("Right")
    keymapGlobal["U0-F"] = Right

    def Left():
        Move("Left")
    keymapGlobal["U0-B"] = Left

    def Up():
        Move("Up")
    keymapGlobal["U0-P"] = Up

    def Down():
        Move("Down")
    keymapGlobal["U0-N"] = Down

    def Home():
        Move("Home")
    keymapGlobal["U0-A"] = Home

    def End():
        Move("End")
    keymapGlobal["U0-E"] = End

    def PageUp():
        Move("PageUp")
    keymapGlobal["U1-V"] = PageUp

    def PageDown():
        Move("PageDown")
    keymapGlobal["U0-V"] = PageDown

    def RightWord():
        Move("C-Right")
    keymapGlobal["U1-F"] = RightWord

    def LeftWord():
        Move("C-Left")
    keymapGlobal["U1-B"] = LeftWord

    def BufferTop():
        Move("C-Home")
    keymapGlobal["U1-S-Comma"] = BufferTop

    def BufferEnd():
        Move("C-End")
    keymapGlobal["U1-S-Period"] = BufferEnd


    # 編集系のキーマップ
    def Return():
        MarkOff()
        keymap.command_InputKey("Return")()
    keymapGlobal["U0-J"] = Return

    def Back():
        MarkOff()
        keymap.command_InputKey("Back")()
    keymapGlobal["U0-H"] = Back

    def Delete():
        MarkOff()
        keymap.command_InputKey("Delete")()
    keymapGlobal["U0-D"] = Delete


    # ショートカット系のキーマップ
    keymapGlobal["U0-Y"] = "C-V"
    keymapGlobal["U0-K"] = "S-End", "C-X"

    def Cut():
        keymap.command_InputKey("C-X")()
        MarkOff()
    keymapGlobal["U0-W"] = Cut

    def Copy():
        keymap.command_InputKey("C-C")()
        MarkOff()
    keymapGlobal["U1-W"] = Copy

    keymapGlobal["U0-S"] = "C-F"

    # マルチストロークのキーマップ
    keymapGlobal["U0-X"] = keymap.defineMultiStrokeKeymap("U0-X")
    keymapGlobal["U0-X"]["U0-S"] = "C-S"


    # ターミナル起動
    def LaunchTerminal():
        keymap.command_ShellExecute("open", "C:\\cygwin64\\bin\\mintty.exe", "-i /Cygwin-Terminal.ico -", "", None)()
    keymapGlobal["U0-U1-T"] = LaunchTerminal
